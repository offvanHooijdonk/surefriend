...
class ClientServiceImpl @Inject constructor(private val clientDao: ClientDao): ClientService {
    ...
    override suspend fun get(id: String): ClientInfo = CoroutineScope(EmptyCoroutineContext).async { clientDao.get(id) }.await()

    override suspend fun list(): Array<ClientInfo> = CoroutineScope(EmptyCoroutineContext).async { clientDao.list() }.await()
}